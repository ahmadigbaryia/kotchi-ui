...
describe("Testing <kui-icon> attributes", () => {
	describe.each([
		...
		["kui-pulled", "kuiPulled", KUIIcon.PullDirection.Right],
	...
	test.each([
		...
		["kui-pulled", KUIIcon.PullDirection.Right],
	...
	describe.each([
		...
		test(`Set ${propertyName} property to ${value} -> ${value ? "adds" : "removes"} '${attributeName}' attribute to the <kui-icon> element`, () => {
			kuiIcon[propertyName] = value;
			expect(kuiIcon.hasAttribute(attributeName)).toBe(value);
		});
		test(`Set ${propertyName} property to ${value} -> ${value ? "adds" : "removes"} className '${className}' to inner icon element`, () => {
			kuiIcon[propertyName] = value;
			expect(kuiIcon.elements.icon.classList.contains(className)).toBe(value);
		});
		test(`Set ${attributeName} attribute to ${value} -> ${value ? "adds" : "removes"} className '${className}' to the inner icon element`, () => {
			kuiIcon.setAttribute(attributeName, `${value}`);
			expect(kuiIcon.elements.icon.classList.contains(className)).toBe(value);
		});
		test(`Set ${attributeName} attribute to ${value} -> ${value ? "adds" : "removes"} '${attributeName}' attribute from the <kui-icon> element`, () => {
			kuiIcon.setAttribute(attributeName, `${value}`);
			expect(kuiIcon.hasAttribute(attributeName)).toBe(value);
		});
	});
	describe.each([
		...
		test(`Tesing invalid value for ${attributeName} attribute`, () => {
			const spy = jest.spyOn(kuiIcon.attributesConfig[attributeName], "attributeChangedHandler");
